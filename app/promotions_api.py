...
def get_promotions():
    ...
    try:
        ...
        for i in promotions:
            ...
            if i.photo1:
                photo1 = url_for('static', filename=f"promotions/{i.photo1}")
            else:
                photo1 = ''
            if i.photo2:
                photo2 = url_for('static', filename=f"promotions/{i.photo2}")
            else:
                photo2 = ''
            if i.photo3:
                photo3 = url_for('static', filename=f"promotions/{i.photo3}")
            else:
                photo3 = ''
            output.append(
                {
                    ...
                    'photo3': photo3,
                    'price': i.price,
                    'date': datetime.datetime.fromtimestamp(i.timestamp).strftime('%m.%d.%Y')
                }
            )
        return current_app.response_class(
            response=json.dumps(
            ...
            output.append(
                {
                    ...
                    'phone': i.phone,
                    'author': user.name + ' ' + user.surname,
                    'photo1': url_for('static', filename=f"news/{i.photo1}") if i.photo1 else '',
                    'photo2': url_for('static', filename=f"news/{i.photo2}") if i.photo2 else '',
                    'photo3': url_for('static', filename=f"news/{i.photo3}") if i.photo3 else '',
                    'address': i.address,
            ...
            output.append(
                {
                    ...
                    'phone': i.phone,
                    'author': user.name + ' ' + user.surname,
                    'photo1': url_for('static', filename=f"promotions/{i.photo1}") if i.photo1 else '',
                    'photo2': url_for('static', filename=f"promotions/{i.photo2}") if i.photo2 else '',
                    'photo3': url_for('static', filename=f"promotions/{i.photo3}") if i.photo3 else '',
                    'address': i.address,
                    'price': i.price,
                    'date': datetime.datetime.fromtimestamp(i.timestamp).strftime('%m.%d.%Y')
                }
...
def get_promotion():
    ...
    try:
        ...
        out = {
            ...
            'author': user.name + ' ' + user.surname,
            'photo1': url_for('static', filename=f"promotions/{promotion.photo1}") if promotion.photo1 else '',
            'photo2': url_for('static', filename=f"promotions/{promotion.photo2}") if promotion.photo1 else '',
            'photo3': url_for('static', filename=f"promotions/{promotion.photo3}") if promotion.photo1 else '',
            'address': promotion.address,
            'orgName': user.org_name,
            'date': datetime.datetime.fromtimestamp(promotion.timestamp).strftime('%m.%d.%Y'),
            'is_your_promotion': 1 if user.id == User.query.filter_by(token=token).first().id else 0
        }
        return current_app.response_class(
            response=json.dumps(
                out
...
def get_new():
    ...
    try:
        ...
        out = {
            ...
            'phone': promotion.phone,
            'author': user.name + ' ' + user.surname,
            'photo1': url_for('static', filename=f"news/{promotion.photo1}"),
            'photo2': url_for('static', filename=f"news/{promotion.photo2}"),
            'photo3': url_for('static', filename=f"news/{promotion.photo3}"),
            'address': promotion.address,
            'orgName': user.org_name,
            'date': datetime.datetime.fromtimestamp(promotion.timestamp).strftime('%m.%d.%Y'),
            'is_your_new': 1 if user.id == User.query.filter_by(token=token).first().id else 0
        }
        return current_app.response_class(
            response=json.dumps(
                out
...
@promotions_api.route('/api/delete_new', methods=['POST'])
def delete_new():
    """
    ---
    post:
    ...
    try:
        ...
        action(token, deviceId, os)
        if user and News.query.filter_by(id=id).first().author == user.id:
            _ = News.query.filter_by(id=id).delete()
            db.session.commit()
            return current_app.response_class(
                response=json.dumps(
                    {'status': 'ok'}
...
@promotions_api.route('/api/delete_promotion', methods=['POST'])
def delete_promotion():
    ...
    try:
        ...
        action(token, deviceId, os)
        if user and Promotions.query.filter_by(id=id).first().author == user.id:
            _ = Promotions.query.filter_by(id=id).delete()
            db.session.commit()
            return current_app.response_class(
                response=json.dumps(
                    {'status': 'ok'}
